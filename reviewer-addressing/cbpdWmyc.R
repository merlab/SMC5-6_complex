...
mycGenes <- c("MYC")
mycGenesDet <- paste0(mycGenes, "_det")
...
df <- as.data.frame(fread("./data/new_onco/PATIENT_DATA_oncoprint-2023-04-17.tsv",
                                    header = TRUE))
df2 <- as.data.frame(fread("./data/new_onco/myc-2023-07-03.tsv",
                                    header = TRUE))
output_alt <- as.data.frame(matrix(nrow = 0, ncol = (27 + 2*length(complexGenes) + 2*length(mycGenes))))
colnames(output_alt) <- c(
             ...
             "instabilityScore", mycGenes, mycGenesDet,
...
#for (i in 3:ncol(df)) {
    ...
    # instability genes #
    ######################
    instabilitySum <- setNames(rep(0, length(mycGenes)), mycGenes)
    instabilitySumDet <- setNames(rep(0, length(mycGenesDet)), mycGenesDet)
    for (k in seq_along(mycGenes)) {
        # this takes all the available alteration info for a gene
        # if anything is altered it would consider the gene altered
        # NOTE: this was modified to remove teh mRNA and protein variations in cbioportal data
        foo <- df2[
                df2$track_name == mycGenes[k] & df2$track_type %in% c("CNA", "MUTATIONS", "STRUCTURAL_VARIANT")
                , i]
...
cbpd$age <- as.numeric(cbpd$age)
for (i in c("isalt", mycGenes, complexGenes)) {
    cbpd[, i] <- as.numeric(cbpd[, i])
}
saveRDS(cbpd, "./reviewer-addressing/cbpdDataWmyc.rds")
print("done")
