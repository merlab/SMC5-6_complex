...
cbpd[, row] <- as.numeric(cbpd[, row])
# cbpd[, row] <- ifelse(cbpd[, row] > 1, "High", "Low")
cbpd[, row] <- ifelse(cbpd[, row] > 2, "High", "Low")
cbpd[, row] <- factor(cbpd[, row], levels = c("Low", "High"))
...
saveRDS(out, "./metabric-brca-instability.rds")
print("done")
table(cbpd[, row])
