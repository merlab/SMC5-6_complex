...
ref_df <- ref_df[ref_df$major == "Endometrial", ]
...
for (current_study in studies) {
    ...
    for (l in 1:length(cols)) {
        ...
        sur_df$group[grepl("missense", sur_df$col, ignore.case = TRUE)] <- "Missense"
...
pdf("./reviewer-addressing/fig4cd-sv-tcga-endometrial.pdf", width = 6, height = 5)
p <- list(
