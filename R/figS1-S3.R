...
db <- readRDS('./results/cbioportal_alt_all.rds')
rownames(db) <- db$name
...
pdf('./figures/figS1-S3.pdf', height = 3, width = 11, onefile = TRUE)
for(i in names(tissueName)) {
    tissue <- i
    mat <- db[db[, 'major'] == i, ]
    mat <- mat[, -ncol(mat)]
    colnames(mat) <- gsub('_det', '', colnames(mat))
    mat <- apply(mat[, 1:ncol(mat)], 2, function(x) return(gsub("\\(putative passenger\\)", "", x)))
    mat <- t(mat)
    mat <- gsub('sv', 'Structural variation', mat)
    mat <- gsub('splice', 'Splice', mat)
    names <- c("Amplification", "Structural variation", "Deep Deletion", 'Inframe Mutation' , 'Missense Mutation', 'Truncating mutation', 'Splice')
    #names <- c("Amplification", "sv", "Deep Deletion", 'Inframe Mutation' , 'Missense Mutation', 'Truncating mutation', 'splice')
    ...
    alter_fun = list(
        ...
        # , 'sv' = alter_graphic("rect", height = 0.33, fill = col["sv"])
        ...
        # , 'Structural variation' = alter_graphic("rect", fill = col["Structural variation"])
        # , 'Missense Mutation' = alter_graphic("rect", fill = col['Missense Mutation'])
    )
    column_title = tissueName[i]

    heatmap_legend_param = list(title = "Genomic variations" 
        , at = names
        , labels = names
    )
    plot(oncoPrint(mat,
...
# db <- db[, c(grep('_det', colnames(db), value = TRUE), 'major')]
# db <- db[, -1]
# pdf('./figures/figS1-S3truncated.pdf', height = 3, width = 6, onefile = TRUE)
# for(i in names(tissueName)) {
#     tissue <- i
#     mat <- db[db[, 'major'] == i, ]
#     mat <- mat[, -ncol(mat)]
