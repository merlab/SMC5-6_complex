...
colnames(output) <- c("tissue_detailed", "name"
...
#for(i in 3:ncol(df)) {
    ...
    sex <- df[df$track_name == "Sex",c(i)]; if (sex == "") sex <- NA
    ...
    if(name %in% colnames(df2)) {
        stage <- df2[df2$track_name == "Neoplasm American Joint Committee on Cancer Clinical Group Stage",name]; if (stage == "") stage <- NA
    } else stage <- NA
    ...
    return(c(tissue_detailed = tissue_detailed, name = name,
...
dim(df)
dim(output)
folder_check('./data/tcga_ov/')
saveRDS(output, "./data/tcga_ov/clinical.rds")
print("done")