...
KM_survival_plot <- function(sur_df, title, censor = TRUE) {
  ...
  x <- length(x)
  if (x == 2) {
    legend.labs <- c("Wild-type", "Amplification")
    colors <- c("#707176", "#DE3B1C")
  }
  if (x == 3) {
...
pdf("./figures/figS3a.pdf", width = 5, height = 5)
...
print(KM_survival_plot(sur_df = sur_df, title = "NSMCE2", censor = TRUE))
dev.off()
print("done")
