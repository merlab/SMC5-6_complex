...
for(i in unique(sectors)) {
  ...
  add_points('male', 3, malec, i, cbpd) # 'cyan'
  add_points('female', 3, femalec, i, cbpd) # 'pink'
  add_points('one', 1, colors[i], i, cbpd)
  # add age
  add_points('under60', 4, under60c, i, cbpd) # '#1b9e77'
  add_points('over60', 4, over60c, i, cbpd) # '#e7298a'
  # add study
  add_points('TCGA' , 2, studytcgac, i, cbpd) # "#d95f02"
  add_points('metabric', 2, studymetabricc, i, cbpd) # '#7570b3'
  # add lines
  for(j in 1:(ymax + 1)) {
