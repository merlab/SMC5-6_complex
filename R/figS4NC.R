pdf('./figures/figS4NC.pdf', width = 12, height = 6, onefile =TRUE)
library(survminer)
library(survival)
library(ggpubr)
library(data.table)
...
KM_plot_PFS <- function(df, title) {
  ...
  pl <- list()
  pl[[1]] <- ggsurvplot(fitisalt, conf.int = TRUE
  ...
  pl[[2]] <- ggsurvplot(fitnsmce2, conf.int = TRUE
                  , risk.table = FALSE, palette = palette, legend = c(0.8,0.95)
                  , pval = FALSE#TRUE
                  , xlab = "Progression free survival (Months)"
                  , ylab = "Probability of Progression free survival"
