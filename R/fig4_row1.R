...
b <- readRDS('./results/metabric-brca/limma.rds')
b_mRNA <- readRDS('./results/metabric-brca/metagx_microarray.rds')
...
calc_gsea(b_rank[order(b_rank, decreasing = TRUE)],
  , rdsout = './results/metabric-brca/pathway.rds'
  , xlsxout = './results/metabric-brca/pathway_enrichment_brca_metabric.xlsx')

breastVolcano <- rmbg(volcanoPlot(b))
breastRainCloud <- rainCloudPlot(b_mRNA[top_genes, ], b_mutation, type = 'microarray')

pdf('./figures/fig4row1.pdf', width = 15, height = 6, onefile = TRUE)
right <- rmbg(ggarrange(plotlist = breastRainCloud[-5], nrow = 2, ncol = 4, align = 'hv'))
left <- rmbg(breastVolcano)
